...
local function getCameraBasis()
	...
	return forwardFlat, rightFlat, Vector3.new(0, 1, 0)
end

local function getMoveIntent(): Vector3
...
local function computeFastOrientation(targetVelocity: Vector3, dt: number): CFrame
	...
	local planar = Vector3.new(targetVelocity.X, 0, targetVelocity.Z)
	local lookDir: Vector3
	if planar.Magnitude > 0.05 then
		lookDir = planar.Unit
	else
		local cLook = Workspace.CurrentCamera and Workspace.CurrentCamera.CFrame.LookVector or hrp.CFrame.LookVector
		lookDir = Vector3.new(cLook.X, 0, cLook.Z)
...
local function flightStep(dt: number)
	...
	local moveIntent = getMoveIntent()
